...
volatile uint16_t game_rounds;
volatile uint16_t correct_button_pressed;
volatile uint16_t sys_tick_count;
volatile uint16_t average_game_time;
volatile uint16_t temp1; 
volatile uint16_t temp2;
volatile uint16_t reaction_time;


typedef enum
{
    GAME_NOEVENT,   /**< State of the game. */
    GAME_BUTTON,    /**< Displays which Button is pressed. */
    GAME_TIMEOUT    /**< When no Button pressed. */
} GAME_event_t;    
